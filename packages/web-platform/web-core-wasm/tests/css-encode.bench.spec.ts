...
import { get_decoded_style_string } from '../binary/encoder.js';
import {
  flattenStyleInfo,
  transformToWebCss,
  genCssContent,
} from '../..//web-mainthread-apis/src/utils/processStyleInfo.js';
...
describe('bench-decode-normal', () => {
  const encodedStyleInfoWASM = encodeCSS(cssMap);
  const encodedStyleInfoJS = JSON.stringify(genStyleInfo(cssMap));
  bench('decode-wasm', () => {
    get_decoded_style_string(encodedStyleInfoWASM, null, true, true);
  });
  bench('decode-js', () => {
...
describe('bench-decode-scope-css', () => {
  ...
  const encodedStyleInfoJS = JSON.stringify(genStyleInfo(cssMap));
  bench('decode-wasm', () => {
    get_decoded_style_string(encodedStyleInfoWASM, null, true, false);
  });
  bench('decode-js', () => {
...
describe('bench-decode-scope-css-and-selector-false', () => {
  ...
  const encodedStyleInfoJS = JSON.stringify(genStyleInfo(cssMap));
  bench('decode-wasm', () => {
    get_decoded_style_string(encodedStyleInfoWASM, null, false, false);
  });
  bench('decode-js', () => {
