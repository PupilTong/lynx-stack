              ...
              if !self.config_remove_css_scope {
                  ...
                  style_sheet.imported_by.iter().enumerate()
                {
                  if imported_by_index == 0 {
                    ...
                    // for other imported_by_css_id, we need to clone the selector
                    let mut new_selector = selector.clone();
                    new_selector.simple_selectors[the_index_of_last_compound_selector].value = format!(
                      "{}=\"{}\"",
                      crate::constants::CSS_ID_ATTRIBUTE,
                      imported_by_css_id
                    );
