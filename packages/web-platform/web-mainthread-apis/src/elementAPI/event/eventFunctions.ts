...
export function createEventFunctions(runtime: MainThreadRuntime) {
  const btsHandler = (event: Event) => {
    ...
    const hname = isCapture
      ? currentTarget[runtimeInfo].eventHandlerMap[lynxEventName]?.capture
        ?.handler
      : currentTarget[runtimeInfo].eventHandlerMap[lynxEventName]?.bind
        ?.handler;
    if (hname) {
  ...
  function __AddEvent(
    ...
    if (newEventHandler) {
      ...
      if (!element[runtimeInfo].eventHandlerMap[eventName]) {
        element[runtimeInfo].eventHandlerMap[eventName] = {
          capture: undefined,
          bind: undefined,
        };
      }
