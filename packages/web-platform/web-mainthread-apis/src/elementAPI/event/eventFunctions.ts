...
export function createEventFunctions(config: {
  ...
  const { publishEvent, getElementByUniqueId } = config;
  const btsHandler = (event: Event) => {
    ...
    if (hname) {
      ...
      const componentId = parentComponent.getAttribute(componentIdAttribute)
        ?? '';
      publishEvent(crossThreadEvent, hname, componentId);
    }
  };
